...
def stats_mysql(stats):
    ...
    for key in stats:
	r = stats[key]
	data = (r.method,r.name,r.num_requests,r.num_failures,r.avg_response_time,r.min_response_time,r.max_response_time,r.current_rps)
	print(data)
        sql = """insert into results (Type, Name, Num_of_reqs, Num_of_failures, Average, Min, Max, Num_of_reqs_per_sec) values ('%s','%s','%s','%s','%s','%s','%s','%s')""" % (r.method,r.name,r.num_requests,r.num_failures,r.avg_response_time,r.min_response_time,r.max_response_time,r.current_rps)
        mysql.execute(sql)
        conn.commit()
        mysql.close()
        conn.close()
...
def save_mysql():
    from runners import locust_runner
    #while True:
    print(locust_runner.state)
    while locust_runner.state != 'stopped': 
         stats_mysql(locust_runner.request_stats);
	 gevent.sleep(2)
